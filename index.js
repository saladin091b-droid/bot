...
(function securityCheck() {
  ...
  let selfFile;
  try {
    selfFile = fs.readFileSync(process.argv[1]);
  } catch {
    console.error("❌ Cannot read self file");
    process.exit(1);
  }
  ...
  const expected = process.env.APP_HASH;
  if (!expected || expected !== hash) {
    console.error("❌ File modified or APP_HASH missing");
    process.exit(1);
  }
})();
...
async function verifyPassword() {
  ...
  // Trial logic - support trial-3h (3 hari) dan trial-30h (30 hari)
  const trialMatch = input.match(/^trial-(\d+)h$/);
  if (trialMatch) {
    ...
    // Jika belum pernah dipakai => aktifkan trial
    if (!lic.used) {
      const now = Date.now();
      const expires = now + trialDays * 24 * 60 * 60 * 1000;
      lic = { used: true, hwid, expired: expires, banned: false, type: "trial", days: trialDays };
      saveLicense(lic);
      console.log(`✔ Trial ${trialDays} hari aktif pada komputer ini.`);
      return true;
    }
    ...
    if (lic.used && !lic.banned) {
      ...
      if (Date.now() <= lic.expired) {
        ...
        if (remainingDays > 1) {
          console.log(`✔ Trial masih aktif (${remainingDays} hari tersisa).`);
        } else {
